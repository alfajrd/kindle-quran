...
re-hashes it) -- `check_m0.py` passes all 33 checks. A2/A3 only prove
...
def test_clean_tree_passes():
    ...
    try:
        ...
        assert len(statuses) == 33, "expected 33 checks to be reported, got %d\n%s" % (len(statuses), stdout)
        assert all(statuses.values()), "expected every check to PASS on an unmodified copy\n%s" % stdout
        assert "RESULT: PASS (33 checks)" in stdout, stdout
    finally:
